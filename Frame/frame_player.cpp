...
void Frame_Player::Signal_Init()
{
    ...
    connect(player, &QMediaPlayer::durationChanged, this , &Frame_Player::slot_set_len);
    ...
    connect(player, &QMediaPlayer::stateChanged, this, &Frame_Player::slot_state_change );
    connect(btn_before, &My_Btn::clicked, this, &Frame_Player::Previous);
    connect(btn_next, &My_Btn::clicked, this, &Frame_Player::Next);
    connect(player, &QMediaPlayer::mediaChanged, this, &Frame_Player::slot_currentMediaChanged);
    connect(btn_mode, &My_BtnPlanMode::signal_modeChanged, [=](int mode){
        //int currentId = _list->currentIndex();
        QMediaPlaylist::PlaybackMode m = (QMediaPlaylist::PlaybackMode)mode;
        _list->setPlaybackMode(QMediaPlaylist::PlaybackMode(mode));
//        player->setPlaylist(_list);
    });
}
...
void Frame_Player::resizeEvent(QResizeEvent *event)
{
    ...
    btn_next->move(btn_play->x() + btn_next->width() + 40 , btn_play->y() + 5);

    btn_mode->move(btn_next->x() + btn_mode->width() + 40 , btn_next->y() + 2);
...
void Frame_Player::play_list(QMediaPlaylist *list, int musicId)
{
    ...
    _list->setPlaybackMode(QMediaPlaylist::PlaybackMode::CurrentItemInLoop);
...
{
    QString tempPath = "";
    Tool_Entity::Music_Entity entity;
    while(!close)
    {
        if(player->state() == 1)
        {
            if(!press_slider)
            {
                ...
                int meida_state = player->mediaStatus();
                 //qDebug()<<meida_state;
                ...
                if(!_list->isEmpty())
                {
                    ...
                    if(tempPath != path)
                    {
                        entity.MediaInfo(path);
                        label_curTitle->setText(entity.Title);
                        label_curPerFormer->setText(entity.Performer);
                        QFont f;
                        f.setFamily("微软雅黑");
                        f.setPointSize(12);
                        QFontMetrics fm(f);
...
void Frame_Player::mouseDoubleClickEvent(QMouseEvent *event)
{
    event->accept();
}
...
void Frame_Player::slot_slider_release()
{
    ...
    player->setPosition(value * MUSIC_DOCK);
    press_slider = false;
}
...
void Frame_Player::set_Size(int size)
{
